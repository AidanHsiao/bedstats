...
window.onload = async function getLeaderboard() {
  ...
  async function loadFriends(totalLeaderboards, settingsData) {
    ...
    if (friendsData === Object.keys(friendsObj).join("")) {
      ...
      await sleep(0);
      friendLeaderboards = JSON.parse(sessionStorage.getItem("friendLeaderboards"));
      apiLoadingFinished(totalLeaderboards, friendLeaderboards, settingsData, friendsObj);
    } else {
      document.getElementById(
        "loaderText"
  ...
  async function apiLoadingFinished(totalLeaderboards, friendLeaderboards, settingsData) {
    ...
    sessionStorage.setItem("friendLeaderboards", JSON.stringify(friendLeaderboards));
    ...
    for (let i = 0; i < 25; i++) {
      ...
      document.getElementById("leaderboards").innerHTML = `${
        ...
        i + 1
      }</div><div class="leaderboardName">${rankData.rank} ${name}</div><div class="leaderboardStars">${stars}</div><div class="leaderboardFKDR">${(
      ...
      if (document.getElementById(`rankPlus${i}`)) {
        document.getElementById(`rankPlus${i}`).style.color = rankData.plusColor;
      }
    }
    await sleep(500);
...
async function changeLeaderboards(changing = true) {
  if (document.getElementById("filter").value === "Friends") {
    if (changing) {
    document.getElementById("titleDiv").innerHTML = "Friends Leaderboard, Sorted By <select id=\"sortType\" name=\"sortType\"><option>Score</option><option>Stars</option><option>FKDR</option><option>BBLR</option><option>Win %</option></select>"
    }
  ...
  document.getElementById("titleDiv").innerHTML = "Global Leaderboard, Sorted By Stars"
  document.getElementById("leaderboards").style.opacity = 0;
  document.getElementById("header").style.opacity = 0;
  document.getElementById("leaderboards").innerHTML = `<div
    class="leaderboardItem"
...
async function changeSort() {
  ...
  switch(sortType) {
    ...
    case "Stars": {
      let baseArray = friendLeaderboards.map((item) => {return item.stats.Bedwars.Experience})
      friendLeaderboards = bubbleSort(baseArray, friendLeaderboards)
      changeLeaderboards(false)
      break;
    }
    case "FKDR": {
      let baseArray = friendLeaderboards.map((item) => {return item.stats.Bedwars.final_kills_bedwars / item.stats.Bedwars.final_deaths_bedwars})
      friendLeaderboards = bubbleSort(baseArray, friendLeaderboards)
      changeLeaderboards(false)
      break;
    }
    case "BBLR": {
      let baseArray = friendLeaderboards.map((item) => {return item.stats.Bedwars.beds_broken_bedwars / item.stats.Bedwars.beds_lost_bedwars})
      friendLeaderboards = bubbleSort(baseArray, friendLeaderboards)
      changeLeaderboards(false)
      break;
    }
    case "Win %": {
      let baseArray = friendLeaderboards.map((item) => {return item.stats.Bedwars.wins_bedwars / item.stats.Bedwars.losses_bedwars})
      friendLeaderboards = bubbleSort(baseArray, friendLeaderboards)
      changeLeaderboards(false)
      break;
    }
