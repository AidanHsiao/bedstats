...
window.electronAPI.sendData('playerData', (event, playerInfo) => {
...
window.electronAPI.sendData('collectedData', async (event, username, playerInfo) => {
  const settingsData = await window.electronAPI.readFile("settings.json")
  pendingUsers.splice(pendingUsers.findIndex(u => u === username), 1)
  document.getElementById("pendingDiv").innerHTML = pendingUsers.join(", ")
  document.getElementById("collectedDiv").innerHTML = ``
  playerInfo.forEach((item) => {
    document.getElementById("collectedDiv").innerHTML = `${document.getElementById("collectedDiv").innerHTML}<div class="collectedItem">
    <div class="itemName">${item.name}</div>
    <div class="itemScore" style="text-shadow: 0px 0px 5px ${scoreToColor(parseFloat(item.score), settingsData.scoreCutoff)}">${item.score}</div>
  </div>`
  })
})
...
document.getElementById("inputBox").addEventListener("keypress", async (key) => {
