...
document
  .getElementById("friendsListWrapper")
  .addEventListener("click", (event) => {
    if (event.target.className.includes("friendRemoveButton")) {
      ...
      const text = document
        .querySelector(`#friendText${eventIndex}`)
...
document
  .getElementById("saveFriendsButton")
  ...
  if (/[A-Za-z\d_]{3,16}/.test(document.getElementById("friendInput").value)) {
    ...
    if (
      !friendsList.includes(
        document.getElementById("friendInput").value.toLowerCase()
      )
    ) {
